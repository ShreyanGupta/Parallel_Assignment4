...
void init(){
	C = vector<long long int>(end-start);
	...
	cudaMalloc((void **)&d_C, C.size() * sizeof(long long int));
	...
	cudaMemcpy(d_C, &C[0], C.size() * sizeof(long long int), cudaMemcpyHostToDevice);
}

void anti_init(){
	cudaMemcpy(&C[0], d_C, C.size() * sizeof(long long int), cudaMemcpyDeviceToHost);
...
__global__ void kernel_complex(
		...
		long long int *C)
{
	__shared__ long long int sum[thread];
	...
	if(row < *dim){
		...
		for(int i=start + lane; i<end; i+=32){
			sum[tid] += (long long int)data[i] * (long long int)B[indices[i]];
		}
	}
	// __syncthreads();
